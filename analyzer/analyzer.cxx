...
int main(int argc, char *argv[])
{
  ...
  INFO("######################################################################");
  INFO("#                                                                    #");
  INFO("# Start executing analyzer version 100                               #");
  INFO("#                                                                    #");
  INFO("######################################################################");
  INFO("");
  ...
  CfgParser * cfgFile = new CfgParser(argv[3]);
  int loglevel = cfgFile->entries["io"].v["LogLevel"];
  INFO("Using log level " << loglevel);
  gLogLevel = loglevel;
  cfgFile->entries["io"].s["Sample"] = argv[1]; //cfgFile.SetValue("Sample", argv[1]);
  ...
  if (cfgFile->entries["io"].s["RemoveBranches"] != "None" && cfgFile->entries["io"].s["RemoveBranches"] != "none") {

    std::vector<string> removeBranches = cfgFile->entries["io"].ss["RemoveBranches"];
