...
extern TH1D * get_cutflow_histogram(const char * runtag);
    ...
    fBTagging(gSystem->ExpandPathName(fCfgFile.entries["btag"].s["BTAG_EffMapFile"].c_str()))
{
  ...
  // analysis type
  fAnalysisType = fCfgFile.entries["ana"].s["AnalysisType"];
  ...
  fTrigger = fCfgFile.entries["ana"].ss["Trigger"];
  fForceUnprescaledTriggers = fCfgFile.entries["ana"].b["ForceUnprescaledTriggers"];
  // maximum number of events to be processed (-1: process all)
  fMaxEvents = fCfgFile.entries["io"].v["MaxEvents"];
  // maximum tree size for output file
  fMaxTreeSize = fCfgFile.entries["io"].v["MaxTreeSize"];
  INFO("Setting maximum tree size to " << fMaxTreeSize);
  // find duplicates?
  fFindDuplicates = fCfgFile.entries["debug"].b["FindDuplicates"];

  // Configure debugging output
  fDumpAll = fCfgFile.entries["debug"].b["DumpAll"];
  fDumpBasic = fCfgFile.entries["debug"].b["DumpBasic"];
  fDumpTruth = fCfgFile.entries["debug"].b["DumpTruth"];
  fDumpTrigger = fCfgFile.entries["debug"].s["DumpTrigger"];

  fSkimActive = fCfgFile.entries["skim"].b["SkimActive"];
  fSkimMuons = fCfgFile.entries["skim"].v["SkimMuons"];
  fSkimMuoptfirst = fCfgFile.entries["skim"].v["SkimMuoptfirst"];
  fSkimMuoptother = fCfgFile.entries["skim"].v["SkimMuoptother"];

  // cuts for T/L ratio
  GetValue(fLooseMuonRelIso, fCfgFile.entries["tlratio"], "LooseMuonRelIso");
  GetValue(fTL_met_max, fCfgFile.entries["tlratio"], "TL_met_max");
  GetValue(fTL_ht_min, fCfgFile.entries["tlratio"], "TL_ht_min");
  GetValue(fTL_jetpt_min, fCfgFile.entries["tlratio"], "TL_jetpt_min");
  GetValue(fTL_nloose_min, fCfgFile.entries["tlratio"], "TL_nloose_min");
  GetValue(fTL_nloose_max, fCfgFile.entries["tlratio"], "TL_nloose_max");
  GetValue(fTL_mumudz_min, fCfgFile.entries["tlratio"], "TL_mumudz_min");
  GetValue(fTL_zmass_min, fCfgFile.entries["tlratio"], "TL_zmass_min");
  GetValue(fTL_zmass_max, fCfgFile.entries["tlratio"], "TL_zmass_max");
  GetValue(fTL_firstmupt_min, fCfgFile.entries["tlratio"], "TL_firstmupt_min");
  GetValue(fTL_mupt_min, fCfgFile.entries["tlratio"], "TL_mupt_min");
  GetValue(fTL_jetdphi_min, fCfgFile.entries["tlratio"], "TL_jetdphi_min");
  GetValue(fTL_mumudphi_max, fCfgFile.entries["tlratio"], "TL_mumudphi_max");
  GetValue(fTL_mt_max, fCfgFile.entries["tlratio"], "TL_mt_max");
  GetValue(fTL_njets_min, fCfgFile.entries["tlratio"], "TL_njets_min");
  ...
  fPileupReweighting = fCfgFile.entries["pu"].b["PileupReweighting"];
  INFO("fPileupReweighting = " << fPileupReweighting);

  const char * PileupDataFile = gSystem->ExpandPathName(fCfgFile.entries["pu"].s["PileupDataFile_2012"].c_str());
  const char * PileupMCFile = gSystem->ExpandPathName(fCfgFile.entries["pu"].s["PileupMCFile_2012"].c_str());
  ...
  //////////////////////////////////////////////////////////////////////
  // FakeRate
  GetValue(fFakeRateMethod, fCfgFile.entries["fake"], "FakeRateMethod");
  if (fFakeRateMethod != "lastbin" && fFakeRateMethod != "zero") {
    THROW("Wrong fake rate method \"" + fFakeRateMethod + "\", use \"lastbin\" or \"zero\"");
  }
  GetValue(fFakeRateDimensions, fCfgFile.entries["fake"], "FakeRateDimensions");
  if (fFakeRateDimensions != 2) { // && fFakeRateDimensions != 3) {
    THROW("Wrong number of fake rate dimensions given");
  }
  if (fAnalysisType == "singlefake" || fAnalysisType == "doublefake") {
    const char * fakeRateFileName = gSystem->ExpandPathName(fCfgFile.entries["fake"].s["FakeRateFile"].c_str());
  ...
  if (fInputType == "data") {
    fLumiRanges = fCfgFile.entries["filters"].s["LumiRanges"];
    if (fLumiRanges != "") {
  ...
  GetValue(fDeltaPhiMin, fCfgFile.entries["ana"], "DeltaPhiMin");
  ...
  if ( !(fCfgFile.entries["systematics"].sub[varName].ss["Values"].empty()) ) {
    for (std::vector<string>::iterator it = fCfgFile.entries["systematics"].sub[varName].ss["Values"].begin();
	 it != fCfgFile.entries["systematics"].sub[varName].ss["Values"].end(); ++it) {
      ...
      INFO("sys:" << a.sys << "  tag:" << a.tag );
      sysStrings.push_back(a);
    }
  }
  else if ( fCfgFile.entries["systematics"].sub[varName].s.count("Values") > 0 ) {
    ssys a = ssys(&var,
		  fCfgFile.entries["systematics"].sub[varName].s.at("Values"),
		  fCfgFile.entries["systematics"].sub[varName].s.at("RunTag"));
    INFO("sys:" << a.sys << "  tag:" << a.tag );
    sysStrings.push_back(a);
  }
...
void Analysis::GetValue(bool & var, cfgEntry & parentName, string varName) {
  var = parentName.b.at(varName);
	 ...
	 it != fCfgFile.entries["systematics"].sub[varName].bb["Values"].end(); ++it) {
      ...
      bsys a = bsys(&var,
		    *it,
		    fCfgFile.entries["systematics"].sub[varName].s.at("RunTag"));
      INFO("sys:" << a.sys << "  tag:" << a.tag );
      sysBools.push_back(a);
    }
  }
  else if ( fCfgFile.entries["systematics"].sub[varName].b.count("Values") > 0 ) {
    bsys a = bsys(&var,
		  fCfgFile.entries["systematics"].sub[varName].b.at("Values"),
		  fCfgFile.entries["systematics"].sub[varName].s.at("RunTag"));
    INFO("sys:" << a.sys << "  tag:" << a.tag );
    sysBools.push_back(a);
  }
...
void Analysis::GetValue(double & var, cfgEntry & parentName, string varName) {
  var = parentName.v.at(varName);
	 ...
	 it != fCfgFile.entries["systematics"].sub[varName].vv["Values"].end(); ++it) {
      ...
      dsys a = dsys(&var,
		    *it,
		    fCfgFile.entries["systematics"].sub[varName].s.at("RunTag"));
      INFO("sys:" << a.sys << "  tag:" << a.tag );
      sysDoubles.push_back(a);
    }
  }
  else if ( fCfgFile.entries["systematics"].sub[varName].v.count("Values") > 0 ) {
    dsys a = dsys(&var,
		  fCfgFile.entries["systematics"].sub[varName].v.at("Values"),
		  fCfgFile.entries["systematics"].sub[varName].s.at("RunTag"));
    INFO("sys:" << a.sys << "  tag:" << a.tag );
    sysDoubles.push_back(a);
  }
...
Bool_t Analysis::MuonCuts (Int_t i) {
  ...
  MuonCuts.FillHistograms(fRunTag.c_str());
...
Bool_t Analysis::JetCuts (Int_t i, vector <int> muons) {
  ...
  JetCuts.FillHistograms(fRunTag.c_str());
					  ...
					  const double HT)
{
  ...
  QCDCuts.Set("nTL_nloose", (fTL_nloose_min <= nMuonsLoose) && (nMuonsLoose < fTL_nloose_max), nMuonsLoose);
  // require some jets
  ...
  // Fill N-1 cuts
  QCDCuts.FillHistograms(fRunTag.c_str());
  // apply cuts
...
void Analysis::Analyze (Long64_t & jentry, EventFilter & hcalevtcfg, lumi::RunLumiRanges & runcfg) {
  ...
  skimCuts.FillHistograms(fRunTag.c_str());
  ...
  for (int i = 0; i < vtx_n; i++) {
    ...
    VertexCuts.FillHistograms(fRunTag.c_str());
  ...
  EventFilterList.FillHistograms(fRunTag.c_str());
  if ( nVertex < 1 || nElectron > 0 || !EventFilterList.PassesAll() ) return;
  ...
  *fSmuon = *fMuon0+*fGaugino;
  ...
  ClearStages();
  AddStageHisto("m_mumu", "m(#mu^{+}, #mu^{-})@GeV", 500, 0., 1000., &m_mumu);
  AddStageHisto("m_gaugino", "gaugino mass m(#mu_{1},j_{1},j_{2})", 25, 0., 1000., &GauginoMass);
  AddStageHisto("m_smuon", "smuon mass m(#mu_{0},#mu_{1},j_{1},j_{2})", 100, 0., 2000., &SmuonMass);
  AddStageHisto("m_smu_chi", "m(#chi): m(#tilde{#mu})", 110, 0., 2200., 25, 0., 500., &SmuonMass, &GauginoMass);

  //////////////////////////////////////////////////////////////////////
  // M(mu, mu) cut
  // needed because no Drell-Yan MC for m(mu,mu) < 10. GeV
  Fill("m_mumu_zpeak", m_mumu);
  if (m_mumu < 15.) {
    return;
  }
  Fill("cutflow", "m_mumu");
...
void Analysis::CreateHistograms()
{
  ...
  histo[Form("%s_cutflow", fRunTag.c_str())] = get_cutflow_histogram(fRunTag.c_str());
...
void Analysis::AddStageHisto (const char * name, const char * xtitle, Int_t nbinsx, Double_t xlow, Double_t xup, Double_t * fillvaluex) {
  stageHistoNames.push_back(name);
  stageHistoXTitles.push_back(xtitle);
  stageHistoNBinsX.push_back(nbinsx);
  stageHistoXLow.push_back(xlow);
  stageHistoXUp.push_back(xup);
  stageHistoNBinsY.push_back(0);
  stageHistoYLow.push_back(0);
  stageHistoYUp.push_back(0);
  stageHistoFillValuesX.push_back(fillvaluex);
  stageHistoFillValuesY.push_back(0);
}

void Analysis::AddStageHisto (const char * name, const char * xtitle, Int_t nbinsx, Double_t xlow, Double_t xup, Int_t nbinsy, Double_t ylow, Double_t yup, Double_t * fillvaluex, Double_t * fillvaluey) {
  stageHistoNames.push_back(name);
  stageHistoXTitles.push_back(xtitle);
  stageHistoNBinsX.push_back(nbinsx);
  stageHistoXLow.push_back(xlow);
  stageHistoXUp.push_back(xup);
  stageHistoNBinsY.push_back(nbinsy);
  stageHistoYLow.push_back(ylow);
  stageHistoYUp.push_back(yup);
  stageHistoFillValuesX.push_back(fillvaluex);
  stageHistoFillValuesY.push_back(fillvaluey);
}
...
void Analysis::RemoveStageHisto (string name) {
  for (unsigned int i = 0; i < stageHistoNames.size(); i++) {
    ...
    if (name == currentName) {
      ...
      stageHistoNBinsX.erase(stageHistoNBinsX.begin()+i);
      stageHistoXLow.erase(stageHistoXLow.begin()+i);
      stageHistoXUp.erase(stageHistoXUp.begin()+i);
      stageHistoNBinsY.erase(stageHistoNBinsY.begin()+i);
      stageHistoYLow.erase(stageHistoYLow.begin()+i);
      stageHistoYUp.erase(stageHistoYUp.begin()+i);
      stageHistoFillValuesX.erase(stageHistoFillValuesX.begin()+i);
      stageHistoFillValuesY.erase(stageHistoFillValuesY.begin()+i);
      return;
    }
  }
  WARNING("Could not remove histogram " + name + " from stage - does not exist?");
}

void Analysis::FillStage (const char * stageTag) {
  if (stageHistoNames[0] == 0) {
    WARNING("There are no stage histograms defined yet!");
    return;
  }
...
void Analysis::ClearStages() {
  ...
  stageHistoXUp.clear();
  stageHistoNBinsY.clear();
  stageHistoYLow.clear();
  stageHistoYUp.clear();
  stageHistoFillValuesX.clear();
  stageHistoFillValuesY.clear();
}
...
{
  // INFO(Form("%s_%s", fRunTag.c_str(), name));
  histo[Form("%s_%s", fRunTag.c_str(), name)] = new TH1D(Form("h1_%s_%s", fRunTag.c_str(), name), title, nbinsx, xlow, xup);
}
...
{
  histo2[Form("%s_%s", fRunTag.c_str(), name)] = new TH2D(Form("h2_%s_%s", fRunTag.c_str(), name), title, nbinsx, xlow, xup, nbinsy, ylow, yup);
}
...
{
  histo2[Form("%s_%s", fRunTag.c_str(), name)] = new TH2D(Form("h2_%s_%s", fRunTag.c_str(), name), title, nbinsx, xbins, nbinsy, ylow, yup);
}
...
{
  histo2[Form("%s_%s", fRunTag.c_str(), name)] = new TH2D(Form("h2_%s_%s", fRunTag.c_str(), name), title, nbinsx, xbins, nbinsy, ybins);
}
...
{
  histo3[Form("%s_%s", fRunTag.c_str(), name)] = new TH3D(Form("h3_%s_%s", fRunTag.c_str(), name), title, nbinsx, xlow, xup, nbinsy, ylow, yup, nbinsz, zlow, zup);
}

void Analysis::Fill(const char * name, double value)
{
  TH1D * h = histo[Form("%s_%s", fRunTag.c_str(), name)];
  //INFO("Finding " << Form("%s_%s", fRunTag.c_str(), name) << " and filling with " << value);
  if (h != 0)
...
{
  TH2D * h = histo2[Form("%s_%s", fRunTag.c_str(), name)];
  if (h != 0)
    histo2[Form("%s_%s", fRunTag.c_str(), name)]->Fill(x, y, global_weight);
  else {
    THROW(std::string("Histogram \"") + name + std::string("\" not existing. Did you misspell or forgot to create?"));
  }
}
...
{
  TH3D * h = histo3[Form("%s_%s", fRunTag.c_str(), name)];
  if (h != 0)
    histo3[Form("%s_%s", fRunTag.c_str(), name)]->Fill(x, y, z, global_weight);
  else {
    THROW(std::string("Histogram \"") + name + std::string("\" not existing. Did you misspell or forgot to create?"));
  }
}
...
void Analysis::PFJetSmearing()
{
  ...
  // do not smear data
  if (fInputType == "data" || fJER_calculation || fRunTag != "0")
    return;
...
{
  TH2D * h = histo2[Form("%s_pfmet_scaled", fRunTag.c_str())];
  if (h == 0)
